...
def compute_window(mode: str,
    ...
    output_file = output_path + str(start_ts) + OUTPUT_SUFFIX
    out_lines = list()
    for dst in covered_destinations:
        for peer in covered_destinations[dst]:
            out_lines.append(OUTPUT_SEPARATOR
                             .join(map(str,
                                       [dst,
                                        peer,
                                        covered_destinations[dst][peer]]))
                             + '\n')
    for dst in uncovered_destinations:
        for peer in uncovered_destinations[dst]:
            out_lines.append(OUTPUT_SEPARATOR
                             .join(map(str,
                                       [dst,
                                        peer,
                                        uncovered_destinations[dst][peer]]))
                             + '\n')
    os.makedirs(output_path, exist_ok=True)
    with open(output_file, 'w') as f:
        f.write('dst peer count\n')
